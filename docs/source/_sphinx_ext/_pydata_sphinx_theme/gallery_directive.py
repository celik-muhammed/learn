...
# -- scikit-plots local patch: shared selection engine, optional -------------
# Filtering, sorting, grouping and pagination live in a first-party package
# so that this directive, `youtube-gallery`, and any future collection
# directive share one implementation.
#
# The import is optional on purpose. A vendored file that hard-imports a
# sibling package stops being vendored: it can no longer be lifted out and
# used on its own, which is the entire reason for keeping a copy here. When
# the engine is absent the directive behaves exactly as upstream does, and
# the selection options report a clear, actionable error instead of being
# silently ignored -- silently ignoring `:filter:` would render the *whole*
# collection where the author asked for three items.
try:
    ...
    class FilterError(ValueError):
        """Placeholder so exception handling parses without the engine."""

#: Options served by the shared engine. Named here so the directive can tell
#: an author precisely which of their options cannot be honoured.
_SELECTION_OPTIONS = ("filter", "sort", "group-by", "limit", "offset", "show-count")
...
_RESERVED_ITEM_KEYS = frozenset({"title", "header", "image", "content"})
# -- end scikit-plots local patch ----------------------------------------------
...
class GalleryGridDirective(SphinxDirective):
    ...
    option_spec: ClassVar[dict[str, Any]] = {
        ...
        # directive needs. The engine lives in `_sphinx_ext.collection` and
        ...
        "search-fields": directives.unchanged,
        # -- end scikit-plots local patch ------------------------------------
    }
    ...
    def _get_source_format(self) -> str:
        ...
        """
        source_path = Path(self.env.doc2path(self.env.docname))
        suffix = source_path.suffix

        source_suffix = self.env.config.source_suffix
        parser_name = (
            source_suffix.get(suffix) if isinstance(source_suffix, dict) else None
        )

        if parser_name == RESTRUCTUREDTEXT or suffix == ".rst":
    ...
    def run(self) -> list[nodes.Node]:
        ...
        if self.arguments:
            ...
            try:
                ...
                return [
                    self.state_machine.reporter.error(
                        f"gallery-grid: could not read {path_data}: {exc}. "
                        f"Data files must be UTF-8 encoded.",
                        line=self.lineno,
                    )
                ]
        ...
        try:
            items = _coerce_items(safe_load(yaml_string), origin)
        except YAMLError as exc:
            return [
                self.state_machine.reporter.error(
                    f"gallery-grid: could not parse YAML from {origin}: {exc}",
                    line=self.lineno,
                )
            ]
        except ValueError as exc:
        ...
        if not _SELECTION_AVAILABLE:
            used = [name for name in _SELECTION_OPTIONS if name in self.options]
            if used:
                return [
                    self.state_machine.reporter.error(
                        f"gallery-grid: the option(s) "
                        f"{', '.join(':' + name + ':' for name in used)} need the "
                        f"'..collection' package, which is "
                        f"not importable in this build. Remove the options, or "
                        f"install/enable that package.",
                        line=self.lineno,
                    )
                ]
            return self._parse(self._render_grid(items, rst=rst))
...
def _register_assets(app: Sphinx) -> None:
    ...
    Notes
    -----
    Silent when the collection package is absent -- the standalone vendored
    build renders plain grids and needs no assets.
    """
