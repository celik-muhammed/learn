...
This directive builds ``docutils`` nodes directly (a bullet list of
paragraphs/references) instead of emitting Markdown or reStructuredText
source text, so it already works unmodified from both MyST (``.md``) and
reStructuredText (``.rst``) pages: use ``` ```{component-list}``` ``` in a
``.md`` file or ``.. component-list::`` in a ``.rst`` file.
"""
