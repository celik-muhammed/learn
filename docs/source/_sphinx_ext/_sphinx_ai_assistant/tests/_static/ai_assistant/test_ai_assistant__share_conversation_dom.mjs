...
artifactRows=sheet.querySelectorAll('.ai-assistant-conv-share-artifact');const directDownloadRow=artifactRows.find(r=>byText(r,'Local save'));const directForget=directDownloadRow?.children.find(c=>c.textContent==='Forget');t('direct toolbar download has Forget',!!directForget);await directForget.click();t('direct toolbar download can be forgotten',sheet._getManagedShareArtifactCount(),directBefore);
// First-class Save file lifecycle is truthful: forget record only, external file remains user-controlled.
const saveDest=sheet.querySelector('[data-key="download"]');t('Save file destination exists',!!saveDest);await saveDest.click();t('Save file primary label is format-specific',primary.textContent,'Save YAML');await primary.click();t('Save file download invoked',downloads.length,downloadsBefore+2);t('Save file filename identifies local-save YAML role',downloads.at(-1)?.name,'ai-conversation-local-save-yaml-20260829.yaml');t('Save file result is explicit',!!byText(sheet,'File saved'));artifactRows=sheet.querySelectorAll('.ai-assistant-conv-share-artifact');const downloadRow=artifactRows.find(r=>byText(r,'Local save'));const forget=downloadRow?.children.find(c=>c.textContent==='Forget');t('download Forget exists',!!forget);await forget.click();t('download record forget notice is truthful',notices.some(n=>n.msg.includes('Delete the file itself from your device')));
