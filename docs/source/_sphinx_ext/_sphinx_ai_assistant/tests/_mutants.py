...
MUTANTS: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "why": "A zero-height row renders its contents overlapping whatever follows. Without the lift the pill sits on the composer instead of above it, and the reserved transcript padding protects nothing.",
        "find": "    transform: translateY(-100%);",
    ...
    },
    {
        ...
        "why": "Collapsed, both ends of the row carry a surface. Left bare when expanded, one end reads as a control and the other as a glyph resting beside it -- the same pair looking like two different kinds of thing at two widths.",
        "find": "    background-color: var(\n        --pst-color-on-background,\n        var(--color-background-secondary, #f5f5f5)\n    );\n    flex: 0 0 auto;",
        "replace": "    flex: 0 0 auto;",
        "harness": "_static/ai_assistant/test_ai_assistant__speak_hint_collapse.mjs",
        "target": "css",
    },
    {
    ...
    {
        "id": "workspace-tab-indicator-drifts-from-the-accent",
        "why": "An indicator with its own colour drifts from the accent used everywhere else in the panel, and a theme that retints the accent leaves this one behind.",
        "find": "    box-shadow: inset 0 -2px 0 0 var(--ai-artifact-accent);",
        "replace": "    box-shadow: inset 0 -2px 0 0 #2980b9;",
        "harness": "_static/ai_assistant/test_ai_assistant__workspace_tabs.mjs",
        "target": "css",
    },
    {
    ...
    },
    {
        ...
        "why": "Collapsed, the row is lifted out of the flow and the space beside the pill shows the conversation. Left in flow when expanded, that same space is panel background -- opaque, with nothing behind it.",
        "find": "    height: 0;\n    min-height: 0;\n    margin: 0 0.75rem;",
        "replace": "    margin: 0 0.75rem 0.5rem;",
        "harness": "_static/ai_assistant/test_ai_assistant__speak_hint_collapse.mjs",
        "target": "css",
    },
    {
    ...
    {
        "id": "artifact-row-overflows-instead-of-wrapping",
        "why": "The row's action buttons do not shrink. Without wrapping they push the row past its container and the artifact name -- the part identifying the row -- is what gets pushed out of view.",
        "find": ".ai-assistant-conv-share-artifact { display:flex; flex-wrap:wrap;",
        "replace": ".ai-assistant-conv-share-artifact { display:flex;",
        "harness": "_static/ai_assistant/test_ai_assistant__share_conversation.mjs",
        "target": "css",
    },
    {
    ...
    },
    {
        ...
        "find": "@container ai-artifact-surface (max-width: 22rem) {",
        "replace": "@media (max-width: 22rem) {",
        "harness": "_static/ai_assistant/test_ai_assistant__activity_and_latest_file_preview.mjs",
        "target": "css",
    },
    {
        "id": "icon-only-label-removed-not-clipped",
        "why": "display:none takes the label out of the box, shrinking the hit area to the glyph and dropping the text a title would otherwise carry. Clipping keeps both while hiding it visually.",
    ...
    {
        "id": "snippet-card-grows-a-second-menu",
        "why": "One builder serves both card surfaces. A second copy is where Escape handling, focus return and outside-click cleanup quietly go missing, because the copy is written for the happy path.",
        "find": "                ], 'ai-md-artifact-overflow');",
