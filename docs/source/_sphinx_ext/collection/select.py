...
def parse_filter(text: str) -> list[FilterTerm]:
    ...
    for raw in _split_terms(text or ""):
        ...
        for token, _ in OPERATORS:
            ...
            if position > 0:
                field_name = raw[:position].strip()
                operand = raw[position + len(token) :].strip().strip("'\"")
                # A mistyped operator (`stars >> 5`) otherwise splits into a
                # valid `>` plus the operand `"> 5"`, which compares as text
                # and quietly returns the wrong rows.
                if operand[:1] in "=<>~^$:!":
                    raise FilterError(
                        f"filter term {raw!r}: unexpected {operand[:1]!r} after "
                        f"the {token!r} operator; use exactly one operator from "
                        f"{[name for name, _ in OPERATORS]}"
                    )
                if not _FIELD_RE.match(field_name):
                    raise FilterError(
                        f"filter term {raw!r}: {field_name!r} is not a valid "
                        f"field name (letters, digits, '_', '-', and '.' for "
                        f"nested fields)"
                    )
        ...
        if not _FIELD_RE.match(field_name):
            raise FilterError(
                f"cannot parse filter term {raw!r}; expected 'field', "
                f"'!field', or 'field OP value' with OP one of "
                f"{[token for token, _ in OPERATORS]}"
            )
...
def group_records(
    ...
    for record in records:
        ...
        labels = [str(atom).strip() for atom in _values_of(value)] if value else []
        for label in labels or [UNGROUPED_LABEL]:
            sections.setdefault(label or UNGROUPED_LABEL, []).append(record)
