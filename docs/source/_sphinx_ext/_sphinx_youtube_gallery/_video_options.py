...
def _align(argument):
    ...
    value=_single(argument)
    if value not in ('left','center','right'):raise ValueError('use left, center or right')
    return value
...
def _query(argument):
    ...
    return ('?'+urlencode(parse_qsl(value,keep_blank_values=True,max_num_fields=128))) if value else ''
...
def player_options(options, title):
    ...
    for key in VIDEO_SPEC:
        ...
        if key=='video-privacy-mode':
            if value in ('false','off','no','0'):continue
            value=''
