...
def collection_id(argument):
    ...
    if not re.fullmatch(r'[A-Za-z][A-Za-z0-9_-]{0,63}', value):
        raise ValueError('collection-id must start with a letter and contain at most 64 letters, digits, underscores or hyphens')
    return value


def field_names(argument):
    """Parse a nonempty, ordered set of safe field paths from an option."""
    names = tuple(dict.fromkeys(part.strip() for part in argument.split(',') if part.strip()))
    if not names or any(not re.fullmatch(r'[A-Za-z_][A-Za-z0-9_-]*(?:\.[A-Za-z_][A-Za-z0-9_-]*)*', name) for name in names):
...
def record_for_browser(item, options):
    ...
    fields = {key: _value(get_field(item, key)) for key in dict.fromkeys((*facets, *sorts))}
    ...
    return {'title':str(item.get('title', '')), 'fields':fields,
...
def metadata_node(records, options):
    ...
    payload = {'version':1, 'interactive':'interactive' in options,
               'facets':list(_field_tuple(options.get('filter-fields', ()))),
               'sorts':list(_field_tuple(options.get('sort-fields', ('title',)))), 'records':records,
               'collectionId':options.get('collection-id', '')}
    encoded = html.escape(json.dumps(payload, ensure_ascii=False, allow_nan=False), quote=True)
    return nodes.raw('', '<span hidden class="sk-collection-data">'+encoded+'</span>', format='html')
