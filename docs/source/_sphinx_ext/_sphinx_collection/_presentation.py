...
def _preserve_source(converter, name):
    ...
    def validate(argument):
        ...
        if name == 'link' or name.startswith('img-') and name not in ('img-alt',):
...
GRID_SPEC = {'grid-'+key: _preserve_source(value, key) for key,value in GridDirective.option_spec.items()}
CARD_SPEC = {'card-'+key: _preserve_source(value, key) for key,value in GridItemCardDirective.option_spec.items()}
