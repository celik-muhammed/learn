...
Returning a structure instead lets each consumer decide, and lets the two
consumers differ *on purpose*: the ``youtube`` directive reads ``video_id``
(the author pointed at one video), while the sync tool prefers
``playlist_id`` (the author pointed at a collection and happened to be
watching one item of it).
...
_KNOWN_TABS = frozenset(
    {
        "videos", "shorts", "streams", "live", "playlists", "podcasts",
        "courses", "releases", "community", "posts", "about", "featured",
        "channels", "store", "search", "membership",
    }
)
...
_RESERVED_SEGMENTS = frozenset(
    {
        "watch", "watch_videos", "watch_popup", "playlist", "embed", "shorts",
        "live", "clip", "post", "results", "hashtag", "feed", "channel", "c",
        "user", "v", "e", "oembed", "redirect", "attribution_link", "premium",
        "about", "account", "upload", "gaming", "music", "movies", "sports",
        "reporthistory", "t", "howyoutubeworks", "creators", "new",
        "playables", "source", "profile", "signin", "logout", "shopping",
    }
)
...
_URL_START_RE = re.compile(r"^(?:[a-z][a-z0-9+.-]*:|//|[?\w@-]+[=/]|[\w@-]+$)", re.I)
...
def _parse_start(value: str) -> int | None:
    ...
    -------
    int or None
        Offset in seconds, or ``None`` when unparseable. An unparseable
...
def _normalize(value: Any) -> str:
    ...
    if "://" not in text and re.match(
        r"^[\w.-]*(?:youtube\.[a-z.]+|youtu\.be|youtubekids\.com)/", text, re.I
    ):
        return "https://" + text
    return text
...
def _handle_handle(ctx: _Context) -> YouTubeReference | None:
    ...
    if not ctx.segments or not ctx.segments[0].startswith("@"):
        return None
    handle = ctx.segments[0][1:]
    if not handle:
        raise ReferenceError(f"{ctx.raw!r}: empty channel handle")
...
def _handle_vanity(ctx: _Context) -> YouTubeReference | None:
    ...
    if not ctx.segments or ctx.head in _RESERVED_SEGMENTS:
        return None
    if ctx.segments[0].startswith("@") or len(ctx.segments) > 2:
        return None
    tab, known = _tab_from(ctx.segments, 1)
    return ctx.base(
...
def _parse_bare(text: str, raw: str) -> YouTubeReference | None:
    ...
    """
    if text.startswith("@"):
        return YouTubeReference(kind=CHANNEL, handle=text[1:], raw=raw)
...
def parse_reference(value: Any) -> YouTubeReference:
    ...
    'cs50'
    >>> parse_reference(
    ...     "https://www.youtube.com/embed/videoseries?list=PLXSX209johrU").kind
    'playlist'
