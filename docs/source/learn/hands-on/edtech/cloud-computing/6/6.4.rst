...
it uses a technique called |named routing| to map a route name to a function.
...
#. First, let's rename ``hello_world()`` function to ``index()``
...
#. Create a new route called ``/hello`` in your ``index.py`` file
   ...
   .. code-block:: python
       :caption: With added ``hello`` route
...
#. Open the page in your browser to verify that it works as expected.
   ...
   .. image:: images/2022-05-19_flask-hello.png

#. Add a new route with a string parameter called ``name`` and then
   ...
   .. image:: images/2022-05-19_flask-hello-jimmy.png

   Here is how your ``index.py`` file should look:

   .. code-block:: python
       :caption: With added ``/hello/name`` route
...
4.2. Templates
------------------------------------------------------------------
Flask uses |Jinja2| as its template engine to provide code functionality
...
#. Create the two directories in your ``quotes-app`` folder with the files we will need
...
#. Edit ``quotes.html`` to include ``name`` variable.
...
#. Edit ``index.py`` to render a template instead of returning HTML code
...
#. Now, we can finish developing the HTML in ``quotes.html``.
...
#. Copy the ``quotes.txt`` to your project folder

   * **Option 1:** Download :download:`quotes.txt <downloads/quotes.txt>`
...
#. Refresh your quotes page, correct any errors, and verify that it displays
   a random quote each time you refresh the page.

   .. image:: images/2022-05-19_flask-quote.png
...
|Jinja2| has a function called ``url_for`` that automatically builds the
...
#. Add the CSS to file ``static/quotes.css`` and then refresh the page.
   ...
   .. image:: images/2022-05-19_flask-quote-css.png

#. Next, we'll upload the ``smilingpython.gif`` smilingpython.gif to the
   ``static`` directory.

   * **Option 1:** Download :download:`smilingpython.gif <downloads/smilingpython.gif>`
...
.. image:: images/2022-05-19_flask-quote-css-image.png
