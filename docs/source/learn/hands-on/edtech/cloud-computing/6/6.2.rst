...
.. Note:: We'll use port **20854** for the reverse proxy.
...
#. Create a folder for your Flask Quotes application
...
#. Verify the information in the ``Dockerfile``.
...
#. Write the ``docker-compose.yml`` file to use ``python/flask:dev`` and
   ...
   Destination port
       ...
       .. Tip:: ``EXPOSE`` is a best-practice marker only. The actual port
   ...
   .. code-block:: yaml
       :caption: ``docker-compose.yml`` template
...
#. Now, can create the ``volume`` with the ``index.py`` to create a basic
