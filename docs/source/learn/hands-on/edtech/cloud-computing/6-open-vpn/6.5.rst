...
.. attention:: The default configuration of OpenVPN uses insecure settings.

For example, OpenVPN uses |cipher BF-CBC| by default, which they then
...
=================================
OpenVPN's community guide on |Hardening OpenVPN| provides recommendations.
Let's see how we can implement some of these features easily.

.. csv-table::
    :header: "Recommendation", "Our Action"
    :widths: 2, 4

    1. Use a strong cipher,Use ``AES-128-CBC`` because it offers security and has a speed |br| advantage over of AES-256.
...
#. Add these lines to the  **client** config.
...
#. Restart the OpenVPN server for the changes to take effect.
   ...
   .. code-block:: bash
       :caption: Output without INSECURE cipher warnings
...
#. Generate a new client.ovpn config. You will notice that it added a new
...
#. Add ``auth SHA256`` to the **client** config.

   * You will have to add the ciphers to the new client config.

#. Connect to the VPN server using your new key.

   * You can verify that it uses the more secure ``SHA256``
     HMAC authentication.

   .. code-block:: bash
      :caption: Output after enabling ``auth SHA256``
   ...
   **Authentication Error**
   ...
   .. figure:: images/tls-auth-error.jpeg

       HMAC error if the client config does not include ``auth SHA256``
