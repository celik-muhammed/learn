...
we connect to the server. There is a documented |configuration error| that
we must correct.

.. Note:: ``push "comp-lzo no"`` causes an error occurs on some OpenVPN
...
.. tip:: View these two resources for additional configuration insights:

    #. |OpenVPN configuration| from stosb.com.
    #. |Server Guide for OpenVPN| from Ubuntu.
...
push
  ...
  * ``push "comp-lzo no"`` disables compression on the client
    ...
    .. caution:: The ``push "comp-lzo"`` directive does not work reliably
