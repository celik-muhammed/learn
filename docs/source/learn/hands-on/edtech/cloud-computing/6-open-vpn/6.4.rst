...
=================================
#. Download the |official OpenVPN community client| if you are using Windows.
   Or, you can locate another OpenVPN client.

   * Or, download
     :download:`OpenVPN-2.5.0-I601-amd64.msi <downloads/OpenVPN-2.5.0-I601-amd64.msi>`
     from this site directly.

#. Download the ``user1.ovpn`` client config from your VPS.
#. Load the ``user1.ovpn`` config in your OpenVPN client.
...
.. note:: This instructions assumes the user is using Windows

1. Open the Command Prompt using ``cmd`` or find it in the application
   list.
#. View the IP address and DNS settings using ``ipconfig``
...
#. Ping the VPN server using the VPN gateway at ``ping 192.168.255.1``
   to verify connectivity to the VPN server.
#. Ping a public IP address, such as 1.1.1.1 or 8.8.8.8 to verify connectivity
   through your VPN server.
#. Perform a ``tracert`` to verify that your connection is going through the
   VPN connection.
...
#. Navigate to |whoer.net|
   ...
   * View the anonymity score and see what affects it.
     ...
     * Select an IP from this |list of public DNS servers| to match
       the country where your VPS is.

       .. figure:: images/whoer-90-percent.png

#. Perform an |IP leak| test to determine if your VPN connection is leaking
...
.. tip:: Look at both the client log and the server log if you have connection issues.
