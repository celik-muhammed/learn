...
#. Let's take another look at Docker Compose commands that we have used.
...
#. Stop the running containers using ``stop``, but don't remove them.

   .. code-block:: bash

       docker-compose stop

#. Try to access the webpage. You will get a `502 Bad Gateway` error.
...
#. Start the project and then refresh the webpage. It will be back where you
   left it.
...
#. Now, let's remove the containers entirely!

   .. Note:: Running this command without having volumes will remove the contain
      and all data.
...
#. View the stopped containers through command ``docker ps -a``.
...
#. Now, rebuild the project using ``docker-compose up -d``
