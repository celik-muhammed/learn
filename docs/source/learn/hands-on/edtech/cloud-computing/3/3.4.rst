...
We'll add |Redis from Docker Hub|.
...
.. tip::
    ...
    using website |YAML Lint|.

#. Add the Redis service to our Wordpress ``docker-compose.yml`` file.

   a. Edit file ``docker-compose.yml``.
   #. Add a new section called ``redis`` between sections ``db``
      and ``wordpress``.
...
#. We need to link it to Wordpress so it can make use it.

   a. Add the reference to ``redis`` under the ``depends_on`` property.
...
#. Let's restart the project and configure Redis.
   ...
   + We can't test Redis easily because we did not configure an outside port.
     ...
     Other Docker containers can access it, but we can't.
...
#. Locate the IP address of the container using
   **docker inspect**:
   ...
   .. note:: Your IP address will be different. The IPAddress key value
   ...
   .. code-block:: bash
       :caption: IP Address of Container
       :emphasize-lines: 1,19,51
...
#. Now we can test Redis using ``redis-cli`` because Redis is
...
#. First of all, let's test the connection using ``ping``.
   ...
   .. error::
       ...
       + You will get a ``Connection refused`` message if something
         ...
         such as trying to use localhost.
...
#. We can use the redis-cli tool to look at all of the information available.
