...
#. Change to the Wordpress folder using ``cd ~/wordpress-docker``
#. Stop the project using the command ``docker-compose down -v``
...
#. Clean up any dangling volumes using
   ``docker volume prune``
#. Physically remove any data stored in the volume directory or
   `db_data` using command ``rm -r db_data``
#. The `wordpress-docker` directory should contain `docker-compose.yml` only.
