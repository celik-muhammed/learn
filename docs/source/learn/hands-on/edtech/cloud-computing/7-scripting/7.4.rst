...
Backing up Data
---------------
.. .. caution:: Data in Docker is fragile.
...
#. Next, create the ``user-backups`` directory
#. Copy the files manually to verify that the command work
   ...
   * ``destination`` is the destination directory or file.
   ...
   .. hint:: Use the full path in the script: use ``/root/my-project`` instead of ``~/my-project``
...
#. Copy them again manually to verify that the commands work without
   user interaction.
#. Then, add the commands to the script.
#. Execute that script to verify that the files were copied to the
   specified directory.
