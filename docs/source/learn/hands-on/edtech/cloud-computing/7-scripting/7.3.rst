...
.. note:: These commands are from :ref:`nginx-commands`

Setting up the Script
---------------------
#. First, let's create a ``scripts`` directory in our home folder.
...
1. Now, let's prompt the user for some information.
  ...
  .. note:: As a personal preference, I capitalize variables to identify
      them easily.
...
1. Now, let's write the site to the configurations file and enable it.
  ...
  .. note:: We have to escape the ``$`` characters that are used in the
...
2. Next, we should ask the user to verify the config for errors before
   ...
   .. note:: The ``if`` condition is true for any character **except**
       ``n`` or ``N``.
