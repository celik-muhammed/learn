...
|GetExitCodeProcess|
    ...
    .. note:: ``GetExitCodeProcess()`` requires additional flags in the process handle.
...
#. Replace the ``0`` in the second arg with any valid unsigned ``int``,
   such as 35.
...
#. Add the ``PROCESS_QUERY_INFORMATION`` flag to ``OpenProcess()``.
#. Use ``GetExitCodeProcess()`` to get the exit code that the process
   returned.
#. Print the exit code from ``GetExitCodeProcess()``.
