...
the :ref:`template C file <basic-c-template>` for this assignment.

Your first task is to test the error handling.

#. Add error handling to ``TerminateProcess()`` and ``CloseHandle()``
...
#. Start notepad.exe and add some text.
   ...
   a. Press the X button to close the windows and verify that the *Save
      Dialog* window displays. Press Cancel to close the dialog |br|
      |image0|

#. Open the *Task Manager* and locate the PID.
#. Use your code to terminate the process using the PID.
#. Verify that notepad closes without prompting the user to save the
   file
...
.. |image0| image:: images/notepad-save-as.png
