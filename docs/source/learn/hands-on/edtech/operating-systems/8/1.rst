...
.. Note:: The file handle must be closed to the file others ``unlink``
...
#. Add error handling to ``unlink()`` by evaluating the return value.
#. Execute the code using an invalid file.
...
#. Verify that the file deletes and ``unlink_status`` has a value of ``0``
...
#. Replace ``unlink`` with ``remove`` and verify that both functions
   operate identically.
...
.. hint::
    *This method is a hack, so don't use it for anything besides testing or
    ...
    #. You can create a file using ``echo`` in the command line
